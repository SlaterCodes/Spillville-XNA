...
namespace Spillville.Utilities
{
    ...
    public class ThreadPoolComponent : GameComponent
    {
		...
		private static List<Thread> _threadReference = new List<Thread>(); 
      ...
      for (int i = 0; i != nThreads_; ++i)
      {
		...
		public void KillAnyRunningThreads()
		{
			if(_threadReference != null)
			{
				foreach (var thread in _threadReference)
				{
            ...
            if (z != null)
            {
                while (w != null)
                {
                    ...
                    catch (System.Exception x)
                    {
                        ...
                        throw new System.Exception("The thread pool user threw an exception on delivery.", x);
                    }
                    w = w.next_;
                }
                lock (this)
